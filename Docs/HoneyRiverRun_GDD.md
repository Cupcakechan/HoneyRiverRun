...
| Riverbanks = instant death | Flower-canyon walls (16×16 tiles) = instant death |
...
| Limited lives, high-score chase | 3 bees, high-score chase with initials entry |
...
Descend the honey river  →  dodge banks, islands & enemies
  ...
  watch the honey tank   →   blast Honeycomb Gates (checkpoints)
        ↑                              ↓
        └──────  crash or run dry  →  lose a bee  ──────┘
                          ↓
              all bees gone → Game Over → enter initials → high score
```
...
- **Grass banks:** x ±5 … ±10 (5 units each side) — lethal on contact
...
```
- MainMenu is always the entry point and the only scene entered first when testing.
...
7. Crash (bank/island/enemy/gate) or empty tank → lose a bee, death feedback, respawn at last Gate.
8. All 3 bees gone → Game Over → score → 3-letter initials if high score → back to Main Menu.
...
One-hit death. Pollen sparkle trail, flapping wings. Constant-drain honey tank.
...
### Fuel — Bloom Orbs
Flying *over* one refills the tank (no shooting). Spawn frequency drops as difficulty climbs.
...
### Hazards / Structures
- **Flower Canyon Walls** — bank tilemaps with trigger colliders. Contact = death.
...
- Lose a bee on: bank, island, enemy, Gate contact, or empty tank.
- Game Over when all 3 bees are spent.
...
**Core — Vertical Slice:** Main Menu + scene management · endless honey-river scroll · Bumble left/right movement · throttle · pooled pollen-dart shooting · lethal canyon walls + death · fuel system · one enemy type · lives (3 bees) + death feedback · score + HUD · Game Over + high-score initials + persistence.
...
6. **Island obstacles** — landmasses in the channel that scroll down and kill on contact (same trigger-death as the banks). Simplest as **pooled scrolling hazards**, reusing the enemy/orb spawner + scroll pattern — independent of river generation, so it can be added shortly after the enemy spawner exists. Primary non-enemy difficulty lever.
...
- **GameManager** (state: Menu/Playing/Paused/GameOver) — lightweight, persistent singleton, assigned from MainMenu.
- **Object pooling** for Pollen Darts, enemies, Bloom Orbs, islands (recycle via `SetActive`; keep activation logic out of `OnEnable` to avoid the construction-time gotcha).
- **Single source of truth** `WorldScroll.Speed` — river, banks, enemies, orbs, islands all read it; throttle + difficulty modify it.
- **ScriptableObjects** for enemy definitions, difficulty tiers, river chunks where it cuts hard-coding.
- **Single responsibility:** scroller, spawner, player movement, throttle, shooter, health, fuel, score, UI are separate components.
- **Physics:** player + hazards use Kinematic Rigidbody2D; lethal geometry uses trigger colliders; player has **Full Kinematic Contacts** enabled so kinematic-kinematic triggers fire.
...
- ✅ **Phase 6 — Flower Canyon Walls + Death** (tilemap trigger colliders, death/respawn)
- ⏳ **Phase 7 — Fuel System** (honey tank drain + Bloom Orb refuel + low-fuel warning)
- **Phase 8 — First Enemy (Ant Bandits)** (spawner + pooled scroll + die to darts) — establishes the spawner reused for orbs & islands
- **Phase 9 — Lives + Death Feedback** (3 bees)
- **Phase 10 — Score + HUD**
- **Phase 11 — Game Over + High Score** (initials + persistence)

**✅ SLICE COMPLETE — fully playable before anything below.**

### Core Expansion (pre-polish)
- **Phase 12 — Honeycomb Gates + Checkpoints**
- **Phase 13 — Progressive Difficulty + World Variety** — **river narrowing/widening/branches via streamed authored chunks**, **island obstacles (pooled scrolling hazards)**, rarer Bloom Orbs, faster/denser enemies, faster baseline scroll — tied to Gates passed. *(Islands may land earlier, right after the Phase 8 spawner, if desired.)*
- **Phase 14 — Full Enemy Roster** (one type at a time)

### Polish
- **Phase 15 — Juice** (particles, screen shake, hit-stop, audio, menu animation)
- **Phase 16 — Final Art + Transitions**
- **Phase 17 — Ship** (WebGL build → `update-HoneyRiverRun.bat` → itch.io)

---

*End of GDD & Roadmap v3 — LOCKED.*
